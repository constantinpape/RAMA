...
thrust::device_vector<int> dCOO::compute_row_offsets(cusparseHandle_t handle, const int rows, const thrust::device_vector<int>& row_ids) const
{
    MEASURE_CUMULATIVE_FUNCTION_EXECUTION_TIME
...
{
    return compute_row_offsets(handle, rows_, row_ids);
}
